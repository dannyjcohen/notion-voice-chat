...
export default function VoiceChat() {
    ...
    try {
      ...
      while (true) {
        ...
        if (sentences.length > 0) {
          ...
          // Using [\s\S]* instead of the dotAll flag (s) for ES2017 compat.
      ...
      // Extract current task from tool call results if we can parse them
      // (The AI text may describe the task — we keep the card if already showing)
  ...
  const handleVADSpeechEnd = useCallback(
    async (audio: Float32Array) => {
      // Pause is handled by VAD internally when startOnLoad=false and we call pause/start
  ...
  const vad = useMicVAD({
    ...
    onSpeechStart: () => {
      // Visual feedback — we're already in listening state
    },
  });
  ...
  const handleTap = useCallback(async () => {
    ...
    try {
      ...
      await ctx.resume();
      // Play a silent 1-sample buffer to fully unlock iOS Safari
